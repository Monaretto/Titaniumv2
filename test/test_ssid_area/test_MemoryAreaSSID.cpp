#include "memory/MemoryAreas/src/MemoryAreaSSID.h"

#include "unity.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"

void test_MemoryAreaSSID() {
    auto result = ESP_FAIL;
    auto memory_area_instance = new MemoryAreaSSID;
    ...
    TEST_ASSERT_EQUAL(memory_area_instance->GetSize(), 32);
    TEST_ASSERT_EQUAL(memory_area_instance->GetAreaIndex(), 1);

    // Write and Read Area
    uint8_t expected_array[32] = {0};
    uint8_t expected_array_read[32] = {0};

    for (int i = 0; i < memory_area_instance->GetSize(); i++) {
        expected_array[i] = (uint8_t)(i % 256);
    }

    TEST_ASSERT_EQUAL(memory_area_instance->Write(expected_array, 32), ESP_OK);
