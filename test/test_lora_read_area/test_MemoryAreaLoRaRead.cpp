...
void test_MemoryAreaLoRaRead() {
    auto result = ESP_FAIL;
    auto memory_area_instance = new MemoryAreaLoRaRead;
    ...
    TEST_ASSERT_EQUAL(memory_area_instance->GetAreaIndex(), 5);
...
void main_test(void) {
    ...
    UNITY_BEGIN();
    RUN_TEST(test_MemoryAreaLoRaRead);
    UNITY_END();
}
